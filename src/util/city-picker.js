(function (wid, dcm) {
    ...
    MultiPicker.prototype = {
        ...
        checkArrDeep: function (parent) {
            var _this = this;
            if ('c' in parent && parent.c.length > 0) {
                _this.jsonArr.push(_this.generateArrData(parent.c));
                _this.checkArrDeep(parent.c[0]);
            }
        ...
        insertLiArr: function (targetUl, arr) {
            ...
            loop(0, arr.length, function (i) {
                html += '<li data-id="' + arr[i].id + '">' + arr[i].v + '</li>';
            });
        ...
        initReady: function (idx, target) {
            ...
            loop(0, _this.idxArr.length, function (i) {
                ...
                _this.resultArr.push({
                    ...
                    "v": _this.jsonArr[i][_this.distance[i] / _this.liHeight + 2].v,
                });
            });
        ...
        checkRange: function (idx) {
            ...
            loop(0, idx + 1, function (i) {
                ...
                tempObj   = i == 0 ? tempObj[targetIdx] : tempObj.c[targetIdx];
            });
            _this.initReady(idx, tempObj);
        },
        initPosition: function (dis, max, idx) {
            // console.log(dis,max,idx);
            dis     = dis < 0 ? 0 : dis;
            dis     = dis > max ? max : dis;
