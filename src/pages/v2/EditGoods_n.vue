<template>
  <div class="editGoods-box">
    <div class="header">
      ...
      <div class="header-cont"><p>编辑商品</p></div>
    </div>
    ...
    <div class="editArea">
      ...
      <div class="colorList">
        ...
        <div class="contain">
          ...
          <div class="item-contain input">
            <input type="text" placeholder="自定义颜色" v-model="newColor" ref="newColor" @keyup="newColorInput"
                   @blur="newColorBlur">
          </div>
      ...
      <div class="sizeList">
        ...
        <div class="contain">
          ...
          <div class="item-contain input">
            <input type="text" placeholder="自定义尺码" v-model="newSize" ref="newSize" @keyup="newSizeInput"
                   @blur="newSizeBlur">
          </div>
    ...
    <div class="selectType selectBox" v-show="typeModalStatus">
      ...
      <div class="list-box">
        <div class="list-contain">
          <div :class="'list-item ' + [form.typeCode===item.typeCode?'sel':'']"
               v-for="(item,index) in commProductTypeList" @click="selectType(index)">
            {{item.typeName}}
          </div>
        </div>
    ...
    <div class="selectStyle selectBox" v-show="styleModalStatus">
      ...
      <div class="list-box">
        <div class="list-contain">
          <div :class="'list-item ' + [styleCode.indexOf(item.styleCode)>-1?'sel':'']"
               v-for="(item,index) in storeStyleList" @click="selectStyle(index)">
            {{item.styleName}}
          </div>
        </div>
    ...
    <div class="selectDiscount selectBox" v-show="selectDiscount">
      ...
      <div class="contain">
        <div class="item" @click="form.isDiscountSupport=1;selectDiscount=false">
          <img class="status" v-if="form.isDiscountSupport===1" src="../../assets/images/common/default_true.png"
               alt="">
          <img class="status" v-else src="../../assets/images/common/default.png" alt="">
          <div class="value">是</div>
        </div>
        <div class="item" @click="form.isDiscountSupport=0;selectDiscount=false">
          <img class="status" v-if="form.isDiscountSupport===0" src="../../assets/images/common/default_true.png"
               alt="">
          <img class="status" v-else src="../../assets/images/common/default.png" alt="">
    ...
    methods: {
      goBack: function () {
        if (this.id === "0") {
          ...
          WebViewJavascriptBridge.callHandler('backToDetail', null, function () {
          });
        }
      },
      //wkwebviewBridge
      ...
      releaseCommodity(){
        ...
        if (this.id === "0") {
          api.saveProduct("post", this.form, this.token).then(res => {
            //接口返回成功
            if (res.returnCode === "0") {
              this.$router.replace("/selectTemplate?productCode=" + res.returnContent.productCode + "&id=" + +res.returnContent.id + "&token=" + this.token);
            }
            //返回失败
            else {
              alert(res.message);
          ...
          api.updateProduct("post", this.form, this.token).then(res => {
            //接口返回成功
            if (res.returnCode === "0") {
              this.$router.replace("/selectTemplate?productCode=" + this.form.productCode + "&id=" + +this.form.id + "&token=" + this.token);
            }
            //返回失败
            else {
              alert(res.message);
